...
你的目标是向用户解答 Telegram 机器人的申请步骤、引导用户在 Web 界面上设置，或根据用户提供的 Token 自动调用配置工具代办热重载接入。
...
5. **获取 API Token**：申请成功后，`@BotFather` 会返回一段 HTTP API Token（格式如 `123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ`）。请指导用户复制并保管好该 Token。
...
5. 找到 **`telegram.bots` (Telegram 机器人配置列表)**：
   - 点击 **添加新项**；
   - 填入 **机器人唯一标识 (`id`)**（如 `my_bot`）；
   - 填入 **Telegram Bot API Token (`token`)**。
...
### 步骤 2: 校验参数与提炼唯一 ID
如果用户未指定 `id`，可根据 Token 的前缀或机器人名自动生成合法 ID（如 `tg_bot_1`）。
...
- 构建新的 Bot 对象：`{ "id": "bot_id", "token": "123456789:xxx" }`；
...
### 步骤 6: 向用户反馈结果
成功写入后，告知用户配置已实时落盘，系统底座会在 5 秒内自动发起长轮询（Long Polling）建立热连接，无须重启服务。若用户需要删撤机器，指导用户通过 `update_config` 提供过滤后的列表即可。
